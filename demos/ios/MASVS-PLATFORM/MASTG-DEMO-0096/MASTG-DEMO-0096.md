...
4. Obtain the code that updates the page content after loading to alter verify whether attacker-controlled input is inserted into the DOM using `innerHTML`.

{{ load_webview_allow_access.r2 # run.sh }}
...
{{ output.txt # ai-decompiled.swift }}
...
4. A payload such as `<iframe src="./secret.txt"></iframe>` causes the WebView to load and display the contents of `secret.txt`.

## Evaluation

The test fails for two reasons:

1. **HTML injection via `innerHTML`:** In `WebViewDelegate.webView(_:didFinish:)`, the user-supplied `firstname` is escaped only for JavaScript string delimiters (`\`, `"`, `\n`), but it's assigned using `innerHTML`. HTML tags such as `<iframe>`, `<img>`, and `<script>` pass through unescaped, allowing the attacker's payload to be interpreted as HTML.
2. **Overly broad `allowingReadAccessTo`:** `docDir` points to the app's `Documents` directory (line 9), and it's passed directly as `allowingReadAccessTo` in `webView.loadFileURL(fileURL, allowingReadAccessTo: docDir)` (line 13 of `showHtmlRegistrationView`). This grants the WebView read access to the entire `Documents` directory, which also contains `secret.txt` (written in `createSecretFile`). An attacker can inject `<iframe src='./secret.txt'></iframe>` as their name to expose this file.
...
To apply the fix to `MastgTest.swift` run the following command:
